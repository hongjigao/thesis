...
wave function compression, based on our CHACI paper \emph{et al.}~\cite{BerardCH2024}.
...
% --- the TikZ tree as its own standalone figure ---
\begin{figure}[htbp]
    \centering
    % if you converted to EPS for latex->dvips, use .eps here
    \includegraphics[width=0.70\columnwidth]{figures/tikz.eps}
    \caption{Left-biased binary tree used by CHACI corner-hierarchical compression: only the left child is further subdivided, while the right child becomes a leaf at each level.}
    \label{fig:chaci-tree}
\end{figure}
...
Therefore, we adopt the \emph{corner-hierarchical matrix} construction, a specialized variant of hierarchical matrices designed to exploit such corner-dominant structure. Rather than refining blocks along a diagonal band, the corner-hierarchical approach recursively subdivides only the corner region containing the dominant coefficients, while treating the complementary part as a leaf that can be compressed more aggressively. This refinement strategy is naturally encoded by the left-biased cluster tree shown in Figure~\ref{fig:chaci-tree}, where at each level only the left child is further expanded and the right child terminates as a leaf.
...
In contrast, CHACI adopts a \emph{corner-driven} recursive subdivision, which can be naturally represented by the left-biased binary tree in Figure~\ref{fig:chaci-tree}. At each refinement step, the upper-left (corner) sub-block is further split (left child), whereas the complementary region is accepted as a leaf block (right child) and compressed adaptively.
